...
TEMPLATE = """
TASK 
You are a mental health professional evaluating how well social media posts express symptoms of {mental_disorder}. Rate how clearly the post reflects that given symptom on a scale from 0 to 5.
...
def load_entailment_dataset(load_dir, load_data):
    ...
    for i in range(len(df)):
        if type(df['description'][i]) != str:
            df_messages.append(df['symptom'][i])
            print(df['symptom'][i])
        else:
            df_messages.append(df['symptom'][i] + ": " + df['description'][i])
    df['message'] = df_messages
    return df
...
def make_batch_dataset(entailment_df, target_disease, init_template, template, input_dir, input_data):
    ...
    for id in range(len(entailment_df)):
        ...
        target_desc = entailment_df['message'][id]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input_data', type=str, default="batch_inputs_sdg_evaluation.jsonl")
    parser.add_argument('--output_dir', type=str, default="./output/")
    parser.add_argument('--output_data', type=str, default="batch_results_sdg_evaluation.jsonl")
