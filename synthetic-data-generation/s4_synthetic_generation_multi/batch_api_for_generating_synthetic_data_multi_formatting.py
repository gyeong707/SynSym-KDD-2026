...
def make_df_result(data, combination):
    print(len(data), len(combination))
    post_list, symp_list, type_list = [], [], []
    error_counts = 0
    for iter, symp in zip(data, combination):
        if iter == {}:
            # print("Empty.")
    ...
    df = pd.DataFrame({'post': post_list, 'type': type_list, 'symptom': symp_list})
    # null 값 개수 확인
    print(df.isnull().sum())
    # null 값 제거
    df = df.dropna()
...
def main(args):
    combination = load_combination(args.load_dir)
    result, error_case = load_batch_result(args.input_dir, args.input_data)
    df = make_df_result(result, combination)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--load_dir', type=str, default="input/batch_results_590.csv")
    parser.add_argument('--input_dir', type=str, default="./output/")
    parser.add_argument('--input_data', type=str, default="batch_results_sdg_multi_depression.jsonl")
    parser.add_argument('--output_dir', type=str, default="./output/")
    parser.add_argument('--output_data', type=str, default="batch_results_sdg_multi_depression_")
